...
\begin{document}
    ...
    \end{array}
    \]
    Thus, with constant \(c=1\) we have \(f(n)\ge \log n\) for all \(n\), so \(f(n)\in \Omega(\log n)\).

    \bigskip

    \textbf{(b) Showing that \(f(n)\) is neither \(\Theta(n)\) nor \(\Theta(\log n)\):}

    \medskip

    Assume by way of contradiction that \(f(n)\in \Theta(n)\). Then there exist constants \(c_1, c_2>0\) and \(n_0\) such that for all \(n\ge n_0\),
    \[
