...
\begin{document}
    ...
    Since \(f(n)\) and \(g(n)\) are nonnegative, by definition of the maximum we have
    \[
    \max(f(n),g(n)) =
    \begin{cases}
    ...
    \]
    for the equality \(\min(f(n),g(n))=\Theta(f(n)+g(n))\) to hold we would also require a constant \(c>0\) such that
    \[
    ...
    f(n)+g(n)=n+1.
    \]
    If we assumed \(\min(f(n),g(n))=\Theta(f(n)+g(n))\), then there would exist constants \(c_1, c_2>0\) and \(n_0\) such that for all \(n\ge n_0\),
    \[
    ...
    1 \ge c_1 (n+1)
    \]
    cannot hold for any fixed \(c_1>0\) as \(n\to\infty\). Hence, the equality does not hold in general and we cannot say that
    \[
