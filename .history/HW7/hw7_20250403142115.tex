...
\begin{document}
    ...
    \subsection*{(a)}
    The greedy algorithm presented in class is optimal for the unweighted interval scheduling problem (i.e., maximizing the number of non-overlapping intervals). However, when each interval \( I_i \) has an associated revenue \( r_i \) and the goal is to maximize
    \[
    \sum_{I_i \in S'} r_i,
    \]
