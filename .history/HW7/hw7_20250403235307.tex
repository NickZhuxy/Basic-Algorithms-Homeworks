...
\begin{document}
    ...
    \subsection*{(a)}
    Huffman encoding is \emph{not} always “length-unique.” While Huffman's algorithm produces an optimal prefix code that minimizes the total weighted code length, it does not always yield a unique assignment of codeword lengths to symbols when there are ties in the input frequencies. In other words, when two or more symbols have the same frequency, different valid choices (i.e., different tie-breakings) during the merging process can lead to different Huffman trees. Although every valid tree has the same total cost, the individual codeword lengths for some symbols may differ from one tree to another.

    \medskip
    \noindent\textbf{Detailed Counterexample:}  
    Consider the set of symbols with frequencies:
    \[
    ...
    \begin{enumerate}
        \item \textbf{Initialization:} List the symbols and their frequencies:
        \[
        A(1),\quad B(1),\quad C(2),\quad D(2).
        \]
        \item \textbf{First Merge:} Combine the two smallest frequencies. Since \(A\) and \(B\) both have frequency 1, merge them to form a new node \(AB\) with weight \(1+1=2\).
    ...
    \begin{itemize}
        \item \textbf{Tree 1:}  
        \begin{enumerate}
            \item Merge \(AB(2)\) and \(C(2)\) to form node \(ABC\) with weight \(2+2=4\).
            \item Merge \(ABC(4)\) with \(D(2)\) to form the root node with weight \(4+2=6\).
        \end{enumerate}
        ...
        \item \textbf{Tree 2:}  
        \begin{enumerate}
            \item Instead of merging \(AB\) with \(C\), merge \(C(2)\) and \(D(2)\) to form node \(CD\) with weight \(2+2=4\).
            \item Merge \(AB(2)\) with \(CD(4)\) to form the root node with weight \(2+4=6\).
        \end{enumerate}
