...
\begin{document}
    ...
    We are given the symbols and frequencies:
    \[
    \begin{array}{c|ccccccc}
    \text{Symbol} & A & B & C & D & E & F & G \\ \hline
    \text{Frequency} & 0.08 & 0.12 & 0.14 & 0.15 & 0.28 & 0.13 & 0.10
    \end{array}
    \]
    
    \subsection*{(a) Derived Huffman Tree}
    
    We first list the symbols with their frequencies:
    \[
    A(0.08),\; G(0.10),\; B(0.12),\; F(0.13),\; C(0.14),\; D(0.15),\; E(0.28)
    \]
    \medskip
    
    \noindent\textbf{Huffman Construction Steps:}
    \begin{enumerate}
        \item Combine \(A(0.08)\) and \(G(0.10)\) to get node \((AG)\) with weight \(0.18\).
        \item Combine \(B(0.12)\) and \(F(0.13)\) to get node \((BF)\) with weight \(0.25\).
        \item Combine \(C(0.14)\) and \(D(0.15)\) to get node \((CD)\) with weight \(0.29\).
        \item Combine \((AG)(0.18)\) and \((BF)(0.25)\) to get node \((AGBF)\) with weight \(0.43\).
        \item Combine \(E(0.28)\) and \((CD)(0.29)\) to get node \((E,CD)\) with weight \(0.57\).
        \item Finally, combine \((AGBF)(0.43)\) and \((E,CD)(0.57)\) to get the root node with weight \(1.0\).
    \end{enumerate}
    
    A possible drawing of the Huffman tree is shown below (using the convention that the left branch is assigned a `0' and the right branch a `1'):
    
    \medskip
    \begin{center}
    \begin{tikzpicture}[sibling distance=3cm, every node/.style={circle,draw,minimum size=1cm}]
    \node {1.0}
        child {node {0.43}
            child {node {0.18}
                child {node {A\\(0.08)}}
    ...
    \subsection*{(b) Mapping from Symbols to Bit Strings}
    
    By assigning a \textbf{0} to every left branch and a \textbf{1} to every right branch, we obtain the following encodings:
    \[
    \begin{array}{c|c}
    ...
    \subsection*{(c) Average Bits per Letter (ABL)}
    
    The average number of bits per letter is computed as:
    \[
    ABL = \sum_{\text{symbol } x} f_x \cdot \ell(x)
    \]
    where \(f_x\) is the frequency and \(\ell(x)\) is the length of the codeword for symbol \(x\).
    ...
    \begin{itemize}
        \item \textbf{Huffman Tree:} (See tree diagram above.)
        \item \textbf{Encoding:}
        \[
        \begin{array}{c|c}
