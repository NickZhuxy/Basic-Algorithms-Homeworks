...
\begin{document}
    ...
    \begin{enumerate}
        ...
        \item We use a memorization / bottom-up approach. There are \((n+1)(S+1)(S+1)=O(nS^2)\) subproblems, and each is computed in \(O(1)\) time. The recurrence is correct by induction, as for each item we consider every valid assignment (or non-assignment) and choose the option that yields the maximum combined valuation.
