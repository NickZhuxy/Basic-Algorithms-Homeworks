...
\begin{document}
    ...
    \section*{Solution}
        ...
        T(0)=T(1)=1.
        \]
        This satisfies the bound \( T(n) \le C (\log n)^2 \) for any constant \( C \) (taking into account that for these small values, the statement is vacuously true or we choose \( C \) large enough to cover these cases).

        \textbf{Inductive Hypothesis:}  
        Assume that for all \( k < n \) (with \( n \) sufficiently large) there exists a constant \( C > \frac{1}{2} \) such that:
        \[
        T(k) \le C (\log k)^2.
        \]
        ...
        \]
        Noting that \( \log \frac{n}{2} = \log n - 1 \), we obtain:
        \[
