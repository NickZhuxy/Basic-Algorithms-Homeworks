...
\begin{document}
    ...
    \textbf{Invariant:} \emph{If there exists an index \( i \) in the original array such that \( A[i] = i \), then there exists an index \( j \) with \( L \leq j \leq R \) (i.e., within the current search interval) such that \( A[j] = j \).}
