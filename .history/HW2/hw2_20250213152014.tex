...
\begin{document}
    ...
    Now we justify the correctness and time Complexity my proposed FindFixedPoint algorithm.
    ...
    We prove correctness by showing that the following invariant is maintained at the start of each iteration of the \textsc{FindFixedPoint} algorithm:

    \smallskip

    \underline{Invariant:} \emph{If there exists an index \( i \) in the original array such that \( A[i] = i \), then there exists an index \( j \) with \( L \leq j \leq R \) (i.e., within the current search interval) such that \( A[j] = j \).}

    \smallskip

    \underline{Initialization:}  
    At the start of the algorithm, we set \( L = 1 \) and \( R = n \). Thus, the entire array is under consideration, and if a fixed point exists it is in \([1,n]\).

    \smallskip

    \underline{Maintenance:}  
    Assume that the invariant holds at the beginning of an iteration with search interval \([L, R]\). Let \( mid = \lfloor (L+R)/2 \rfloor \). We consider three cases:

    \begin{enumerate}
        \item \textbf{Case 1: \( A[mid] = mid \).}  
        In this case, we have found a fixed point and the algorithm returns \( mid \), so the invariant guarantees correctness.
        
        \item \textbf{Case 2: \( A[mid] > mid \).}  
        Define the function \( f(i) = A[i] - i \). Since \( A \) is strictly increasing, \( f(i) \) is also strictly increasing. Thus, for any index \( j > mid \), we have
        \[
        f(j) > f(mid) > 0,
        \]
        which implies \( A[j] - j > 0 \) or \( A[j] > j \). Therefore, no index \( j \) with \( j > mid \) can be a fixed point. Hence, if a fixed point exists, it must lie in the left half, i.e., in the interval \([L, mid-1]\). The algorithm sets \( R \gets mid-1 \), and the invariant is maintained.
        
        \item \textbf{Case 3: \( A[mid] < mid \).}  
        Similarly, for any index \( j < mid \), the strict monotonicity of \( f \) implies
        \[
        f(j) < f(mid) < 0,
        \]
        so \( A[j] - j < 0 \) or \( A[j] < j \). Thus, no index \( j \) with \( j < mid \) can be a fixed point, and if a fixed point exists, it must be in the right half, i.e., in \([mid+1, R]\). The algorithm then sets \( L \gets mid+1 \), and the invariant holds.
    \end{enumerate}

    \smallskip

    \underline{Termination:}  
    The loop terminates when \( L > R \). At this point, by the invariant, if there were a fixed point in the original array, it would have been contained in \([L,R]\). Since the interval is now empty, no fixed point exists, and the algorithm correctly returns \(\bot\).
