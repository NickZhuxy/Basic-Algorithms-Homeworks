...
\begin{document}
    ...
    \textbf{(b) }
    Assume that more than $\frac{n}{2}$ phones are working. We now describe an algorithm that finds a working phone using $O(n)$ queries to the machine $M$, and we incorporate two key points:
    \begin{enumerate}
        \item Even if two malfunctioning phones are paired and produce $M(A,B)=11$, this does not disrupt the overall algorithm.
        \item We use the hint given in class: if you have $A > B$ and $C \leq B$, then $A - C > B - D$, to show that the majority of working phones is preserved through the elimination process.
    \end{enumerate}
    ...
    \paragraph{Majority Preservation via the Professor's Hint.}
