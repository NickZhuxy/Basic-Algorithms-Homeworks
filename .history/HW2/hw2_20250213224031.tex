...
\begin{document}
    ...
    Assume that more than $n/2$ phones are working. We describe an algorithm that finds a working phone using $O(n)$ queries to the machine $M$.

    \subsection*{Algorithm}
    \begin{enumerate}
        \item \textbf{Pairing:} Arbitrarily pair the $n$ phones. (If $n$ is odd, leave one phone unpaired.)
        \item \textbf{Testing:} For each pair $(A,B)$, query $M(A,B)$.
        \begin{itemize}
            \item If $M(A,B) = 11$, then \emph{at least one} of $A$ or $B$ is working. Retain one of these phones (say, $A$) as a candidate.
            \item If $M(A,B) = 10$ or $00$, then at least one phone in the pair is malfunctioning. Discard both phones.
        \end{itemize}
        \item \textbf{Handling Unpaired Phone:} If there is an unpaired phone, include it in the candidate set.
        \item \textbf{Iteration:} Replace the original set with the candidate set and repeat the pairing and testing process until only one candidate remains.
    \end{enumerate}

    \subsection*{Correctness}
    \begin{itemize}
        \item \textbf{Working Pairs:} If both phones in a pair are working, then $M(A,B) = 11$, and retaining one ensures that a working phone is preserved.
        \item \textbf{Mixed Pairs:} If one phone is working and the other malfunctioning, the working phone will correctly identify the malfunctioning one, producing a response of $10$ (or $00$ if the malfunctioning phone lies), and both are discarded. Although a working phone is lost, the majority condition guarantees that at least one working phone survives in some other pair.
        \item \textbf{Majority Invariant:} Since more than half of the phones are working initially, the process of pairing and discarding (or retaining one from each pair with a $11$ output) preserves the property that the candidate set contains at least one working phone.
    \end{itemize}

    \subsection*{Time Complexity}
    In the first round, at most $\lfloor n/2 \rfloor$ queries are made. In each subsequent round, the number of candidates is reduced by a constant factor. Hence, the total number of queries is bounded by:
    \[
    \frac{n}{2} + \frac{n}{4} + \frac{n}{8} + \cdots \le n,
    \]
    which is $O(n)$.
    ...
    Assume now that fewer than $n/2$ phones are working. We claim that \textbf{no procedure using $M$ is guaranteed to find a working phone}.

    \subsection*{Justification}
    \begin{itemize}
        \item \textbf{Adversarial Responses:} When the majority of phones are malfunctioning, the malfunctioning phones can respond arbitrarily. In particular, they can collude and always output $1$ (claiming the other phone is working) regardless of the true status.
        \item \textbf{Indistinguishability:} With such adversarial behavior, consider two malfunctioning phones. If paired, they may both output $1$, resulting in $M(A,B) = 11$, which is identical to the response when both phones are actually working.
        \item \textbf{Consequence:} Any algorithm that uses $M$ to distinguish working from malfunctioning phones cannot reliably differentiate between a pair of malfunctioning phones and a pair of working phones if the adversary’s responses mimic correct behavior. Therefore, the algorithm cannot guarantee that a phone identified as working is indeed working.
    \end{itemize}

    Thus, when fewer than $n/2$ phones are working, the possibility of adversarial responses by the malfunctioning phones prevents the design of a procedure using $M$ that is guaranteed to find a working phone.
