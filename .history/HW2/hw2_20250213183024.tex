...
\begin{document}
    ...
    Since there are \(O(1)\) rounds (in fact, \(O(\log n)\) rounds, but the constant factors add up to \(O(n)\) overall), and the final verification step takes \(O(n)\) queries, the total number of queries is \(O(n)\).
