...
\begin{document}
    ...
    \section*{solution}
        We are given:
        \[
        T(0)=T(1)=1, \quad\text{and} \quad T(n)=T\Big(\Big\lceil\frac{n}{2}\Big\rceil-1\Big)+\log n \quad \text{for } n\ge2.
        \]
        We wish to show that \( T(n)=O((\log n)^2) \).
        
        \textbf{Inductive Hypothesis:} Assume that for all \( k < n \) (with \( n \) sufficiently large) there exists a constant \( C > \frac{1}{2} \) such that:
        \[
        T(k) \le C (\log k)^2.
        \]
        ...
        \textbf{Base Case:} For \( n=0 \) and \( n=1 \), we have \( T(0)=T(1)=1 \). By choosing \( C \) sufficiently large or by considering the claim only for \( n\ge2 \), the base case holds.
