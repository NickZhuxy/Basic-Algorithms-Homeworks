...
\begin{document}
    ...
    \begin{Solution}
        \textbf{(a)} The runtime of radix sort is given by 
        \[
        \Theta(d(n+k)),
        \]
        where:
        \begin{itemize}
            \item $n$ is the number of sensor readings (or elements) to be sorted.
            \item $k$ is the range of each “digit” in the counting sort subroutine (i.e., the number of possible values a digit can take).
            \item $d$ is the number of digits (or passes) required to represent the numbers in the chosen base, which is determined by $d = \log_k M$, where $M$ is the range of sensor readings.
        \end{itemize}
        
        \textbf{(b)} To optimize the total runtime $\Theta(d(n+k))$, we choose $k$ (and hence $d$) appropriately for each case.
        ...
        \begin{enumerate}[(i)]
            ...
            Using Stirling's approximation, we have $\log (n!) = \Theta(n\log n)$. Again, set $k = \Theta(n)$, so that
            \[
