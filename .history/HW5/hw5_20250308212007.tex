...
\begin{document}
    ...
    \section*{Solution}
        ...
        RodCuttingWithoutRepetition(n, p):
            // Initialize a table T[0..n][0..n]
            for j = 0 to n:
                T[0][j] = 0           // Revenue for rod of length 0 is 0
            for i = 1 to n:
                T[i][0] = -∞          // With no pieces, revenue is -∞ (or a very small number)
        
            // Fill in the table
            for j = 1 to n:           // Consider pieces of length 1 to n
                for i = 1 to n:       // For each rod length i
                    if i >= j:
                        // Option 1: do not use the piece of length j
                        // Option 2: use the piece of length j (if it fits)
                        T[i][j] = max( T[i][j-1],  p[j] + T[i-j][j-1] )
                    else:
                        T[i][j] = T[i][j-1]
        
            return T[n][n]          // Maximum revenue for rod length n using pieces 1..n
        \end{verbatim}
