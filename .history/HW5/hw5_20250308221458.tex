...
\begin{document}
    ...
    %Write your solution here!
    \section*{Solution}
        \textbf{(a)} \emph{Optimal Play on the Instance \(A = [1,1,1,2,2,2]\):}
        
        If you move first on the row \(A = [1,1,1,2,2,2]\), a detailed analysis using the recurrence for \(\mathsf{MaxGain}(i,j)\) shows that you can guarantee a payoff of \(\mathbf{5}\). 
        
        \smallskip
        
        \textbf{Brief Strategy Description:}  
        On your first move, you should take the last coin (with value 2). This move forces the remaining coins into a configuration where, regardless of whether your opponent picks from the beginning or the end, you can continue with moves that secure you a total gain of 5. In other words, by starting with the coin of value 2 at the right end, you limit your opponent’s options so that his optimal counterplay still leaves you with a favorable subarray from which your subsequent moves can extract additional value.
        ...
        \begin{algorithmic}[1]
            ...
            \FOR{\(i \gets 1\) \textbf{to} \(n\)}
                \STATE \(\texttt{DP}[i][i] \gets v_i\) \COMMENT{Base case: only one coin}
            \ENDFOR
            \FOR{\(i \gets 1\) \textbf{to} \(n-1\)}
                \STATE \(\texttt{DP}[i][i+1] \gets \max\{v_i, v_{i+1}\}\) \COMMENT{Base case: two coins}
            \ENDFOR
            \FOR{\(length \gets 3\) \textbf{to} \(n\)}
                \FOR{\(i \gets 1\) \textbf{to} \(n - length + 1\)}
