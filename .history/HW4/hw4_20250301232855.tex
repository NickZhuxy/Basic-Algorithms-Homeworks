...
\begin{document}
    ...
    To prove that our algorithm uses \( n-2 \) comparisons in the worst case, we provide a worst-case input where \( A[1] \) does not match any element from \( A[2] \) to \( A[9] \), forcing the algorithm to make all \( 8 \) comparisons (since \( 10-2 = 8 \)).
