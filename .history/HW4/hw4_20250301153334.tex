...
\begin{document}
    ...
    \[
    T(n) \le T\Bigl(\frac{n}{3}\Bigr) + T\Bigl(\frac{2n}{3}\Bigr) + c\,n,
    \]
    where \(c\,n\) represents the linear-time work (partitioning, grouping, etc.) done at the current level.

    \medskip

    \textbf{Building the Recursion Tree:}  
    At the root (level 0), the cost is \(c\,n\). This node branches into two subproblems of sizes \(\frac{n}{3}\) and \(\frac{2n}{3}\).

    At level 1, the costs at the nodes are:
    \[
    c\left(\frac{n}{3}\right) \quad \text{and} \quad c\left(\frac{2n}{3}\right).
    \]
    Thus, the total cost at level 1 is
    \[
    c\left(\frac{n}{3} + \frac{2n}{3}\right) = c\,n.
    \]
