...
\begin{document}
    ...
    \textbf{Correctness:}  
    Every element in the array \(A\) appears exactly twice. The algorithm sequentially compares \(A[i]\) with \(A[i-2]\) for \(i = 3\) to \(n\). If a match is found, the duplicate pair is returned immediately. If no match is found during these \(n-2\) comparisons, then by the structure of the input, the only possibility is that the duplicate pair appears at the end of the array; hence, the algorithm returns the indices \((n-1,\, n)\).
    ...
    \]
    where \(a,\, b,\, c,\, d,\, e\) are distinct values. The algorithm proceeds as follows:
    \begin{itemize}
        \item \(\boldsymbol{i=3}\): Compare \(A[3]=c\) with \(A[1]=a\) \(\rightarrow\) No match.
        \item \(\boldsymbol{i=4}\): Compare \(A[4]=d\) with \(A[2]=b\) \(\rightarrow\) No match.
        \item \(\boldsymbol{i=5}\): Compare \(A[5]=b\) with \(A[3]=c\) \(\rightarrow\) No match.
        \item \(\boldsymbol{i=6}\): Compare \(A[6]=c\) with \(A[4]=d\) \(\rightarrow\) No match.
        \item \(\boldsymbol{i=7}\): Compare \(A[7]=d\) with \(A[5]=b\) \(\rightarrow\) No match.
        \item \(\boldsymbol{i=8}\): Compare \(A[8]=e\) with \(A[6]=c\) \(\rightarrow\) No match.
        \item \(\boldsymbol{i=9}\): Compare \(A[9]=a\) with \(A[7]=d\) \(\rightarrow\) No match.
        \item \(\boldsymbol{i=10}\): Compare \(A[10]=e\) with \(A[8]=e\) \(\rightarrow\) Match found.
    \end{itemize}
    
    In this case, the algorithm performs \(10-2=8\) comparisons before finding a duplicate. This input is tailored for our algorithm so that the duplicate is only discovered in the final comparison.
