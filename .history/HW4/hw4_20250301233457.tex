...
\begin{document}
    ...
    \begin{enumerate}
        ...
        \end{itemize}
    \end{enumerate}
    It is not necessary to consider cases where one or both elements are already in \(b\) because once an element is in \(b\), it has already been eliminated as a candidate for either the minimum or maximum. 
    The only comparisons that affect progress toward isolating the two extreme elements are those that move an element from \(a\) to \(c\) or from \(c\) to \(b\) (eliminated). 
