...
\begin{document}
    ...
    The recurrence taught in class for the groups-of-3 variant is
    \[
    T(n)= T\left(\frac{n}{3}\right) + T\left(\frac{2n}{3}\right) + O(n).
    \]
    This recurrence reflects that after grouping and finding medians in constant time per element, the algorithm recursively processes one subproblem of size roughly \(\frac{n}{3}\) and another of size roughly \(\frac{2n}{3}\), plus a linear amount of work for partitioning and grouping.
    
    \medskip
    
    \textbf{Using the Recursion-Tree Method:}  
    At the root level, the cost is \(O(n)\). At the next level, the two recursive calls contribute costs proportional to \(O\left(\frac{n}{3}\right)\) and \(O\left(\frac{2n}{3}\right)\), whose sum is still \(O(n)\). In fact, each level of the recursion tree incurs a total cost of \(O(n)\).  
    ...
    At each level of the recursion tree, regardless of how the problem sizes split, the sum of the sizes of the subproblems equals \(n\) (since \(\frac{n}{3} + \frac{2n}{3} = n\)). Therefore, the total cost incurred at every level is \(c\,n\).

    \medskip

    \textbf{Height of the Recursion Tree:}  
    The depth of the recursion is determined by the size of the largest subproblem. Notice that the larger subproblem is of size \(\frac{2n}{3}\). To find the height \(h\), we solve:
    \[
    \left(\frac{2}{3}\right)^h n \approx 1.
    \]
    Taking logarithms, we have
    \[
    h \approx \log_{3/2} n = \Theta(\log n).
    \]

    \medskip

    \textbf{Total Cost:}  
    Since each level costs \(c\,n\) and there are \(\Theta(\log n)\) levels, the total running time is:
    \[
    T(n) = \Theta(n \log n).
    \]

    Because the recurrence solves to \(\Theta(n \log n)\), the deterministic selection algorithm using groups of size 3 does \emph{not} run in linear time. Hence, we cannot argue that it is linear; rather, its worst-case running time is superlinear.
