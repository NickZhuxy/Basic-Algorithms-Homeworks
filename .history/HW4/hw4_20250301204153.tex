...
\begin{document}
    ...
    \]
    Here, the work to form groups, compute each group’s median, and partition around the pivot is still \(O(n)\).
    
    \medskip
    
    \textbf{Using the Recursion-Tree Method:}  
    At the top level, the cost is \(O(n)\). The recursive calls now split the problem into one subproblem of size \(\frac{n}{7}\) and another of size \(\frac{5n}{7}\). Notice that the sum of the fractions is
    \[
    \frac{1}{7} + \frac{5}{7} = \frac{6}{7} < 1.
    \]
    This means that the total size of the subproblems shrinks by a constant factor at each level, and the cost at every level remains \(O(n)\) while the depth of the recursion is bounded by a constant factor (in fact, the recursion tree converges to a total cost of a constant multiple of \(n\)).
    
    \medskip
    
    Thus, summing over all levels, we have
    \[
