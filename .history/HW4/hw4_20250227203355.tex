...
\begin{document}
    ...
    We prove that any comparison-based algorithm for finding both the minimum and maximum in an array of \( n \) distinct integers must perform at least 
    \[
    \frac{3n}{2} - O(1)
    \]
    comparisons in the worst case. The proof uses an adversary argument that tracks three parameters throughout the algorithm:
    
    \begin{itemize}
        \item \(a\): the number of elements that have not been compared at all.
        \item \(b\): the number of elements that have been compared in such a way that they have \emph{both} won and lost at least one comparison.
        \item \(c\): the number of elements that have been compared at least once but have \emph{only} one “mark” (i.e., only wins or only losses).
    \end{itemize}
    
    We now analyze the algorithm in four parts.
